...
var wordsSettings = {
	...
	bgColorCorrect:'#5D7599', //Correct letter, in the right place
	bgColorPositionActive:'#fff', //yellow
	bgColorPosition:'#9CADC8', //Correct letter, in the wrong place
	
	bgColorNotExistActive:'#fff',
	bgColorNotExist:'#B5BDC1', //Letter not in the word
...
var textDisplay = {
					...
					resultTitleComplete:'CONGRATULATIONS\n You have received 5\n entries for the draw.\n You will be contacted\n if you are selected as the \nwinner.',
					resultTitleOver:'Thanks for playing.\n you have been entered\n into the draw.\n You will be contacted\n if you are selected as the\n winner.',
					resultTimer:'',
					resultDesc:'',
					resultWord:'',
				}
...
function buildGameButton(){
	...
	buttonContinue.addEventListener("click", function(evt) {
		...
		let param = searchParams.get('cookieemail');
		let email = atob(param);
		//alert(email);
...
// redirect function
function redirect() {
	window.location.href = "https://www.oshawacentre.com/events-sales/";
...
function findWord(){
	...
	gameData.answer = $('#daily_word').val();
...
function displayStatus(status){
	...
	/*	var now = new Date();
		...
		document.cookie = "gamecomplete=complete;" + expires + ";path=/";
//alert(expires);
...
function completegame(email){
	//alert(email);
	$.ajax({
		type:"post",
		url:'../completegame.php',
		data:{email:email},
		cache:false,
		success:function(data){
			//alert('done');
...
function incompletegame(email){
	//alert(email);
	$.ajax({
		type:"post",
		url:'../incompletegame.php',
		data:{email:email},
		cache:false,
		success:function(data){
			//alert('done');
